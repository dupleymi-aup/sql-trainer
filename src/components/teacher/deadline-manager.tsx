...
function getTimeStatus(dueAt: number): { label: string; variant: 'destructive' | 'default' | 'secondary' } {
  ...
  return { label: t('reminder.hoursLeft', { hours: String(Math.round(hoursLeft)) }), variant: 'secondary' };
}
...
export function TeacherDeadlineManager() {
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/admin/deadlines/${deleteId}`, { method: 'DELETE', headers: {} });
      const data = await res.json();
